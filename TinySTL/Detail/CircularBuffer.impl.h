...
namespace TinySTL
{
    namespace Detail
    {
        ...
        {
            --index;
            index = (index == -1 ? index + N : index); // 循环
            return index;
        }
        template <class T, size_t N, class Alloc>
    ...
    template <class T, size_t N, class Alloc> // 移动赋值
    ...
    template <class T, size_t N, class Alloc> // 删除最旧的元素
    ...
    template <class T, size_t N, class Alloc> // 输出流，依次打印缓冲区数据
    std::ostream &operator<<(std::ostream &os,  circular_buffer<T, N, Alloc> &cb)
    {
        typename circular_buffer<T, N, Alloc>::size_type size = cb.size();
        if (!cb.empty())
        {
            os << "(";
    ...
    template <class T, size_t N, class Alloc> // 清空缓冲区数据
    ...
    { // only for ctor
        ...
        {                                                                          // N > n
            finish_ = TinySTL::uninitialized_fill_n(start_, n, val);               // 填充n个
            finish_ = TinySTL::uninitialized_fill_n(finish_, N - n, value_type()); // 剩余部分填充默认值
            indexOfTail = n - 1;
            size_ = n;
        }
    }
    template <class T, size_t N, class Alloc>
    template <class InputIterator>
    void circular_buffer<T, N, Alloc>::allocateAndCopy(InputIterator first, InputIterator last)
    { // only for ctor
        int n = last - first;
        start_ = dataAllocator::allocate(N);
        indexOfHead = 0;
        if (N <= static_cast<decltype(N)>(n))
        {
