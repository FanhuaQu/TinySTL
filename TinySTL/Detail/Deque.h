...
namespace TinySTL
{
    ...
    namespace Detail
    {
        ...
        public:
            ...
            dq_iter(size_t index, T *cur, cntrPtr container) : mapIndex_(index), cur_(cur), container_(container) {}
            ...
            typename iterator<bidirectional_iterator_tag, T>:: reference operator*() { return *cur_; }
            typename iterator<bidirectional_iterator_tag, T>:: reference operator*() const { return *cur_; }
            typename iterator<bidirectional_iterator_tag, T>:: pointer operator->() { return &(operator*()); }
            typename iterator<bidirectional_iterator_tag, T>:: pointer operator->() const { return &(operator*()); }
            dq_iter &operator++();
        ...
        public:
            template <class T1>
            friend dq_iter<T1> operator+(const dq_iter<T1> &it, typename dq_iter<T1>::difference_type n);
            template <class T1>
            friend dq_iter<T1> operator+(typename dq_iter<T1>::difference_type n, const dq_iter<T1> &it);
            template <class T1>
            friend dq_iter<T1> operator-(const dq_iter<T1> &it, typename dq_iter<T1>::difference_type n);
            template <class T1>
            friend dq_iter<T1> operator-(typename dq_iter<T1>::difference_type n, const dq_iter<T1> &it);
            template <class T1>
            friend typename dq_iter<T1>::difference_type operator-(const dq_iter<T1> &it1, const dq_iter<T1> &it2);
            template <class T1>
            friend void swap(dq_iter<T1> &lhs, dq_iter<T1> &rhs);
        };
