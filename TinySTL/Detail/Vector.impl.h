...
namespace TinySTL
{
    ...
    { // 传入的是右值
        if (this != &v)
        {
            ...
            finish_ = v.finish_;
            endOfStorage_ = v.endOfStorage_;
            v.start_ = v.finish_ = v.endOfStorage_ = 0; // 注意这个
        }
        return *this;
    }
    ...
    void vector<T, Alloc>::resize(size_type n, value_type val)
    {
        if (n < size())
        {
            dataAllocator::destroy(start_ + n, finish_); // 收缩
            finish_ = start_ + n;
        }
    ...
    { // 和resize的区别在于，reserve是真正改变capacity，而resize改变的是包含元素的区间大小
        if (n <= capacity())
            return;
    ...
    void vector<T, Alloc>::reallocateAndCopy(iterator position, InputIterator first, InputIterator last)
    {
        ...
        T *newStart = dataAllocator::allocate(newCapacity); // 分配内存
        T *newEndOfStorage = newStart + newCapacity;
        T *newFinish = TinySTL::uninitialized_copy(begin(), position, newStart); // 复制开始到插入点位置的元素
        newFinish = TinySTL::uninitialized_copy(first, last, newFinish);         // 复制插入的元素
        newFinish = TinySTL::uninitialized_copy(position, end(), newFinish);     // 复制插入点到末尾的元素

        destroyAndDeallocateAll(); // 销毁旧内存块
        start_ = newStart;
        finish_ = newFinish;
    ...
    void vector<T, Alloc>::insert_aux(iterator position, InputIterator first, InputIterator last, std::false_type)
    {
        ...
        if (locationLeft >= locationNeed) // 空间够用
        {
            if (finish_ - position > locationNeed)
            {
                TinySTL::uninitialized_copy(finish_ - locationNeed, finish_, finish_); // 先将finish_前面locationNeed长度的元素复制到末尾
                std::copy_backward(position, finish_ - locationNeed, finish_);         // 剩余的元素往后移动，这样前面就腾出了locationNeed的空间
                std::copy(first, last, position);
            }
            else
            {
                iterator temp = TinySTL::uninitialized_copy(first + (finish_ - position), last, finish_); // 未初始化空间中复制一段新元素
                TinySTL::uninitialized_copy(position, finish_, temp);                                     // 插入点后面的元素复制到新位置
                std::copy(first, first + (finish_ - position), position);                                 // 复制剩余的插入元素
            }
