#ifndef _REVERSE_ITERATOR_H__
#define _REVERSE_ITERATOR_H__
...
namespace TinySTL
{
    ...
    public:
        // 构造、复制、析构
        reverse_iterator_t() : base_(0), cur_(0) {} // 默认构造函数

        explicit reverse_iterator_t(const iterator_type &it) : base_(it) // 从给定迭代器构造反向迭代器
        {
            auto temp = it;
            cur_ = --temp;
        }
    ...
    public:
        ...
        template <class Iter>
        friend reverse_iterator_t<Iter> operator+(typename reverse_iterator_t<Iter>::difference_type n, const reverse_iterator_t<Iter> &rev_it);
        template <class Iter>
        friend typename reverse_iterator_t<Iter>::difference_type operator-(const reverse_iterator_t<Iter> &lhs, const reverse_iterator_t<Iter> &rhs);
