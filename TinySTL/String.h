...
namespace TinySTL
{
    ...
    public:
        ...
        string(InputIterator first, InputIterator second);
    ...
    {
        start_ = dataAllocator::allocate(last - first);         // 开辟内存
        finish_ = TinySTL::uninitialized_copy(first, last, start_); // 将范围内的元素复制到start_指向的内存
        endOfStorage_ = finish_;
    }
    template <class InputIterator>
